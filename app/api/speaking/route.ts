...
// 正确方式：通过 protos 获取 AudioEncoding
  ...
  try {
    ...
    if (!audioFile || !text) {
      return NextResponse.json(
        { error: 'Missing audio file or text data' },
        { status: 400 }
      );
    }
    ...
    const config = {
      encoding: AudioEncoding.WEBM_OPUS, // 与前端 MediaRecorder 一致
    ...
    const prompt = `
      ...
      2. "fluencyScore" (0-100)
      3. "wordDetails": array of { "word", "accuracy": 0-100, "feedback": string }
    ...
    try {
      pronunciationResult = JSON.parse(aiText);
    } catch (parseError) {
      console.error('Gemini JSON parse error:', parseError, 'Raw:', aiText);
      return NextResponse.json(
